...
from .environment import reinformer_qcar_eval, OfflineWaypointEnv
...
def evaluator(
    ...
    rewards_mean, _, _, _ = reinformer_qcar_eval(
        model=model,
        device=DEVICE,
    ...
    return env.get_normalized_score(rewards_mean) * 100
...
def experiment(variant):
    ...
    params_json: bytes = json.dumps(params)
    ...
    normalized_reward_list: List[float] = []
    for _ in range(1, max_train_iters+1):
        ...
        if use_wandb:
            wandb.log(
                data={
                        "training/time" : t2 - t1,
                        "evaluation/score" : normalized_reward,
    ...
    if use_wandb:
        wandb.log(
            data={
                "evaluation/max_score" : max(normalized_reward_list),
                "evaluation/last_score" : normalized_reward_list[-1]
            }
        )
    print(normalized_reward_list)
    print("=" * 60)
