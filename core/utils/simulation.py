...
def prepare_test_environment(node_sequence: list) -> np.ndarray:
    roadmap: ACCRoadMap = ACCRoadMap()
    x_pos, y_pose, angle = roadmap.nodes[node_sequence[0]].pose
...
def prepare_test_environment_waypoint(waypoint_index: int, node_sequence: list) -> None:
    roadmap: ACCRoadMap = ACCRoadMap()
    waypoint_sequence = roadmap.generate_path(node_sequence)
