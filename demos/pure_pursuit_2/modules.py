...
from .client import OptitrackClient
...
class PurePursuiteCar(PhysicalCar): # for simulation purpose
    def __init__(
        ...
        self.client: OptitrackClient = OptitrackClient(self.data_queue)
    ...
    def setup(self, node_sequence: List[int], waypoints: np.ndarray, init_waypoint_index: int = 0) -> None:
        ...
        self.client.mount()
        time.sleep(4) # wait for threads to start
    ...
    def update_state(self, image: np.ndarray) -> None:
        # get the ego state from the qlabs/optitrack
    ...
    def execute(self, task_queue: MPQueue = None, obs_queue: MPQueue = None) -> None:
        ...
        if image is not None:
            ...
            print(f"current waypoint: {self.waypoints[self.current_waypoint_index]}")
            print(f"current position: {self.ego_state[:2]}")
